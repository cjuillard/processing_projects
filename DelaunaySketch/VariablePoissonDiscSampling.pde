...
class VariablePoissonDiscSampling {
  ...
  VariablePoissonDiscSampling(float worldWidth, float worldHeight, float minRadius, float maxRadius, RadiusProvider radiusProvider) {
  ...
  private void expandStep() {
    ...
    float radius = radiusProvider.getRadius(curr.x, curr.y);
    ...
    for(int i = 0; i < samples; i++) {
      ...
      int gridOffsetSize = ceil(maxRadius / minRadius);
      boolean validSample = true;
      for(int rOff = -gridOffsetSize; rOff <= gridOffsetSize; rOff++) {
       int currRow = row + rOff; 
       if(currRow < 0 || currRow >= rows)
         continue;
         
       for(int cOff = -gridOffsetSize; cOff <= gridOffsetSize; cOff++) {
         ...
         float neighborRadius = radiusProvider.getRadius(neighbor.x, neighbor.y);
         if(d < radius && d < neighborRadius) {
           validSample = false;
         }
       }
      }
